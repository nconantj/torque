...
if (isset($session_id)) {
    // For the merge function, we need to find out, what would be the next
    // session
    ...
    // Create array of Latitude/Longitude strings in JSON format for Leaflet
    $mapdata = array();

    while ($geo = $db->get_row_array_assoc($sessionqry)) {
        if (( $geo["lat"] != 0 ) && ( $geo["lon"] != 0 )) {
            $mapdata[] = array($geo['lat'], $geo['lon']);
        }
    }
    ...
    $session_id = "";
    $imapdata = json_encode(array(array(37.235, -115.8111)));
    $setZoomManually = 1;
}
...
<html lang="en">
    <head>
        ...
        <meta name="author" content="Nathanial Jones">
        ...
        if ($setZoomManually === 0) {
            ...
            <script language="javascript" type="text/javascript">
                $(document).ready(function(){
                    ...
                    mymap.fitBounds(path_polyline.getBounds());
